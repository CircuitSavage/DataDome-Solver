...
var chrome148APIsPresent = map[string]struct{}{
...
var chromeFeatures = map[string]bool{
	"pro_t": true, "wglo": true, "prso": true, "wbst": true, "psn": true,
	"edp": true, "addt": true, "wsdc": true, "ccsr": true, "nuad": true,
	"bcda": false, "idn": true, "capi": false, "svde": false, "vpbq": true,
}
...
func generateNavTiming() map[string]any {
	...
	ttrdDenom := connectEnd - tcp
	if ttrdDenom <= 0 {
		ttrdDenom = 1
	}
	var ntTtrd any
	ttrdNumer := responseTime - ttrdDenom
	if connectEnd-tcp > 0 {
		ntTtrd = roundFloat(ttrdNumer/ttrdDenom, 6)
	}

	return map[string]any{
		"nt_tcp": roundFloat(tcp, 1), "nt_dns": roundFloat(dns, 1),
		"nt_rd": 0, "nt_irt": roundFloat(firstInterim, 6),
		"nt_rt": roundFloat(responseTime, 6), "nt_tls": roundFloat(tls, 6),
		"nt_ttf": roundFloat(responseTime+swTime+10, 6),
		...
		"nt_ttrd": ntTtrd, "nt_le": roundFloat(loadEventDuration, 1),
		"nt_dcle": roundFloat(dclDuration, 6), "nt_di": roundFloat(domInteractive, 6),
...
func generateBchk() string {
	...
	for i, api := range bchkAPIs {
		if _, ok := chrome148APIsPresent[api]; ok {
